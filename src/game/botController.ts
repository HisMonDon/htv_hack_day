...
export function advanceBotLane(
  ...
  const phaseTimeRemainingSeconds = Math.max(0, laneState.phaseTimeRemainingSeconds - deltaSeconds);

  if (laneState.phase === "pick") {
    const options = laneState.pendingAbilityOptions ?? nextAbilityOptions;
    if (phaseTimeRemainingSeconds > 5 - BOT_PICK_DELAY_SECONDS) {
      return { ...laneState, abilityCooldownRemainingSeconds: cooldowns, phaseTimeRemainingSeconds };
    }

    const pick = decideBotAbilityPick(options, laneState.equippedAbilities);
    return {
      ...laneState,
      equippedAbilities: [options[pick], laneState.equippedAbilities[1]],
      pendingAbilityOptions: null,
      phase: "combat",
      phaseTimeRemainingSeconds: 15,
      waveNumber: laneState.waveNumber + 1,
      abilityCooldownRemainingSeconds: [0, cooldowns[1]],
    };
  }
  ...
  if (phaseTimeRemainingSeconds === 0) {
    return {
      ...
      phase: "pick",
      phaseTimeRemainingSeconds: 5,
      pendingAbilityOptions: nextAbilityOptions,
    };
  }
